    ...
    <div className="container nav-wrap">
      ...
      <nav ref={navRef} className="primary-nav mfsys-editorial-nav" aria-label="Primary">
        ...
        <div className="nav-menu"><button className="nav-menu-trigger" aria-expanded={open === "industries"} onClick={() => toggle("industries")}>Industries <span>⌄</span></button>{open === "industries" && <div className="mfsys-editorial-mega industries-menu">
          <section className="editorial-intro industries-intro"><div className="editorial-art-wrap"><IndustryVisual/></div><div className="editorial-copy"><span className="editorial-eyebrow">OUR INDUSTRIES</span><h2>Digital solutions for a more inclusive and sustainable world.</h2><p>Enabling financial inclusion, climate resilience and smarter supply chains across key sectors.</p><Link href="/industries" onClick={closeAll}>Explore All Industries <b>→</b></Link></div><div className="editorial-proof"><span><b>30+</b>Financial Institutions</span><span><b>12+</b>Countries</span><span><b>100+</b>Professionals</span></div></section>
          <section className="industry-cards">
            {industries.slice(0, 6).map((name, i) => (
              <Link href="/industries" className={`industry-card industry-card-${i}`} key={name} onClick={closeAll}>
                <div className="industry-card-art"><span>0{i + 1}</span><i /></div>
                <div><strong>{name}</strong><small>{["Inclusive and digital financial services","Shariah-compliant banking and finance","Finance for farmers and value chains","Carbon markets and climate resilience","Connected and efficient supply chains","Digital solutions for greater impact"][i]}</small></div>
                <em>›</em>
              </Link>
            ))}
          </section>
